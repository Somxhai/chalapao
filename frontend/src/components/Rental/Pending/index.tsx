...
const Pending = () => {
	...
	const otherImages = item?.item_images?.slice(1, 4) || [];
	...
	return (
		<main className="container mx-auto px-4 lg:px-16 py-8 text-gray-700">
			<div className="flex justify-center items-center gap-6 mb-10 text-sm">
				{"1234".split("").map((step, i) => (
					<React.Fragment key={`step-${step}`}>
						<div
							className={`flex items-center gap-1 ${+step <= 2 ? "" : "text-gray-400"}`}
						>
							<div
								className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${+step <= 2 ? "bg-black text-white" : "border border-gray-400"}`}
							>
								{step}
							</div>
							<span
								className={`${+step <= 2 ? "text-black font-semibold" : "text-gray-400"}`}
							>
								{
									[
										"Details",
						...
						{step !== "4" && (
							<hr
								className={`w-8 border-t ${+step <= 1 ? "border-black" : "border-gray-300"}`}
							/>
						)}
					</React.Fragment>
				))}
			</div>
			<h1 className="text-3xl font-bold mb-2 text-left">Confirming</h1>
			<div className="mb-10 max-w-xl mx-auto">
				<div
					className={`rounded-xl py-6 px-8 w-full text-center ${
			...
			<div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
				<div className="lg:col-span-2 bg-white p-4 shadow rounded space-y-4 w-full">
					...
					<div className="text-sm">
						...
						<div className="flex justify-between font-bold border-t pt-2 mt-2">
							<span>รวม</span>
							<span>
								{item ? item.price_per_day * 30 + 120 : 0} บาท
							</span>
						</div>
					</div>
				</div>
				...
				<div className="lg:col-span-3 flex flex-col gap-6">
					<div className="flex flex-col md:flex-row gap-6">
						<div className="w-full md:w-2/5 bg-white p-4 shadow rounded space-y-2">
						...
						<div className="w-full md:w-3/5 bg-white p-4 shadow rounded space-y-2">
					...
					<div className="bg-white p-4 shadow rounded space-y-2">
						...
						<p>
							ถึงวันที่ : {formatThaiDate(rental?.end_date ?? "")}
						</p>
					</div>
				</div>
			</div>
			...
			<div className="flex justify-end mt-6">
				<button className="bg-gray-300 text-black px-6 py-2 rounded hover:opacity-90">
					Cancel
				</button>
			</div>
		</main>
